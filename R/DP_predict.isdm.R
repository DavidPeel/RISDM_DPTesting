...
predict.isdm <- function( object, covars, habitatArea=NULL, S=500, intercept.terms=NULL, n.threads=NULL, n.batches=1, includeRandom=TRUE, includeFixed=TRUE, includeBias=FALSE, type="intensity", confidence.level=0.95, quick=FALSE, scaleup=1, DaveQuickTest=0, DP.mem.clean=F,...){
  ...
  else if (DaveQuickTest==2)
  {
    ...
    lambdaRaster <- terra::rast(cbind(predcoords, lambda.qs[,2]), crs = terra::crs(covars), type = "xyz")
    
    lambdaRaster <- c(lambdaRaster, terra::rast(cbind(predcoords, lambda.qs[,1]), crs = terra::crs(covars), type = "xyz"))
    lambdaRaster <- c(lambdaRaster, terra::rast(cbind(predcoords,  lambda.qs[,3]), crs = terra::crs(covars), type = "xyz"))
    if (DP.mem.clean)
    {
      rm(lambda.qs)
      gc()
    ...
    lambda.mean <- matrixStats::rowMeans2(mu.all, na.rm = T)
    ...
    lambda.sd <- matrixStats::rowSds(mu.all, na.rm = T)
